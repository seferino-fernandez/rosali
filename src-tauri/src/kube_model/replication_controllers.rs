...
impl From<ReplicationController> for KubeReplicationController {
  fn from(replication_controller: ReplicationController) -> Self {
    ...
    KubeReplicationController {
      name: replication_controller.metadata.name.clone().unwrap_or_default(),
      namespace: replication_controller.metadata.namespace.clone().unwrap_or_default(),
      age: utils::to_age(replication_controller.metadata.creation_timestamp.as_ref(), Utc::now()),
      desired,
      current,
      ready,
      containers,
