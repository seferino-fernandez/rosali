  ...
  fn from(pod: Pod) -> Self {
    ...
    let (status, containers_ready, restarts, containers_count, containers) = match &pod.status {
      Some(status) => {
        ...
        (get_status(status, &pod), containers_ready_counter, restart_counter, containers_count, containers)
      }
...
impl KubeContainer {
  pub fn from_api(
    ...
    if let Some(container_statuses) = container_statuses_ref {
      if let Some(container_status) = container_statuses.iter().find(|cs| cs.name == container.name) {
  ...
  match container_state {
    Some(state) => {
      if let Some(container_state_waiting) = state.waiting {
        container_state_waiting.reason.unwrap_or_else(|| "Waiting".into())
      } else if let Some(container_state_terminated) = state.terminated {
        container_state_terminated.reason.unwrap_or_else(|| "Terminating".into())
          ...
          Some(container_state) => {
            if let Some(container_state_terminated) = &container_state.terminated {
              if container_state_terminated.exit_code == 0 {
                "".into()
              } else if container_state_terminated.reason.as_ref().unwrap_or(&String::default()).is_empty() {
                format!("Init:{}", container_state_terminated.reason.as_ref().unwrap())
              } else if container_state_terminated.signal.unwrap_or_default() != 0 {
                format!("Init:Signal:{}", container_state_terminated.signal.unwrap())
              } else {
...
fn is_pod_init(container_state_waiting: Option<ContainerStateWaiting>) -> bool {
  container_state_waiting.map(|state_waiting| state_waiting.reason.unwrap_or_default() != "PodInitializing")
    .unwrap_or_default()
}
