...
impl From<Ingress> for KubeIngress {
    fn from(ingress: Ingress) -> Self {
        let rules = ingress.spec.as_ref().and_then(|spec| {
            spec.rules.as_ref().map(|rules| {
                rules.iter().map(|rule| {
                    let paths = rule.http.as_ref().and_then(|http| {
                        Some(http.paths.iter().map(|path| {
